...
from src.luggage.luggage_monitor import LuggageMonitor
from src.luggage.visualizer import Visualizer
from src.tracking.bytetrack import ByteTracker


PERSON_CLASSES = {"person"}
LUGGAGE_CLASSES = {"backpack", "bag", "suitcase"}
VALID_CLASSES = PERSON_CLASSES | LUGGAGE_CLASSES

DEFAULT_VIDEO_PATH = "data/avss2007_luggage.mp4"
DEFAULT_CONFIG_PATH = "configs/luggage.yaml"
DEFAULT_HOMOGRAPHY_PATH = "configs/homography_luggage.json"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Lance le use case bagages abandonnés en local avec affichage OpenCV."
    )
    ...
    while True:
        if not paused:
            ...
            detections = [
                ...
                if d.class_name in VALID_CLASSES
            ]
            ...
            for t in tracks:
                name = category_name(t, class_names)
            ...
            statuses = monitor.update(bags, persons)
